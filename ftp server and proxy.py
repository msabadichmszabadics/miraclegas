...
ftp_folder = ''  # Global variable to store the selected folder for FTP
...
# Function to get global IP using requests
def get_global_ip():
    try:
...
def start_ftp_server(listbox):
    global ftp_folder
    if not ftp_folder:
        listbox.insert(tk.END, "[!] No folder selected for FTP server.\n")
        listbox.yview(tk.END)
        return

    authorizer = DummyAuthorizer()
    authorizer.add_anonymous(ftp_folder, perm='elradfmw')  # Full permissions

    handler = FTPHandler
    handler.authorizer = authorizer
    handler.max_cons = 25  # Limit maximum connections
    handler.passive_ports = range(30000, 30010)  # Use passive port range

    global_ip = get_global_ip()
    ftp_server = FTPServer((global_ip, 21), handler)  # Serve on global IP address
    listbox.insert(tk.END, f"[*] FTP server started on {global_ip}:21 serving {ftp_folder}\n")
    listbox.yview(tk.END)
...
def select_folder():
    global ftp_folder
    ftp_folder = filedialog.askdirectory()
    if ftp_folder:
        print(f"Selected folder for FTP server: {ftp_folder}")

def create_gui():
    root = tk.Tk()
    root.title("Proxy Server with FTP")
    ...
    select_folder_button = tk.Button(root, text="Select FTP Folder", command=select_folder)
    select_folder_button.pack()

    start_ftp_button = tk.Button(root, text="Start FTP Server", command=lambda: start_ftp_server(log_listbox))
    start_ftp_button.pack()
